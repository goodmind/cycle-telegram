...
var messageLife = function (_a) {
    ...
    return (startDate - (update.message || update.channel_post || update.edited_channel_post).date * 1000) <= 30000;
};
...
function makeSources(state) {
    ...
    var startDate = state
        .pluck('startDate')
        .share();
    var messageLike = function (kind) {
        return rx_1.Observable.zip(updates, startDate)
            .filter(ramda_1.pipe(ramda_1.head, ramda_1.propIs(types_1.Message, kind)))
            .filter(messageLife)
            .map(ramda_1.head)
            .share();
    };
