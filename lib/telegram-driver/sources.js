...
var max = function max(property) {
  return function (acc, current) {
    return current[property] > acc ? current[property] : acc;
  };
};

var makeUpdatesResolver = function makeUpdatesResolver(token) {
  return function (offset) {
    return (0, _apiRequest.makeAPIRequest)({
      ...
      return _rx2.default.Observable.fromArray(res);
    });
  };
};
...
function makeUpdates(token) {
  ...
  return _rx2.default.Observable.return(initialState).expand(function (_ref) {
    ...
    return resolve(offset).toArray().combineLatest(_rx2.default.Observable.interval(100).take(1), function (updates, _) {
