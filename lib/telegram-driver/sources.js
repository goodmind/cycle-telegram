...
function makeWebHook(token, action) {
  ...
  return _rx2.default.Observable.concat(_rx2.default.Observable.just(initialState), webHookUpdates.map(function (updates) {
    return (0, _types.UpdatesState)({
      startDate: initialState.startDate,
      offset: (0, _ramda.reduce)(max('update_id'), 0, updates) + 1,
      updates: updates
...
function makeSources(state) {
  var updates = state.pluck('updates').share().map(function (u) {
