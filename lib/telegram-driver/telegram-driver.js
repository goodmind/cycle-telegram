...
function makeResponsesSelector(res) {
    return function responses(_a) {
        var _b = _a === void 0 ? {} : _a, responseType = _b.responseType, method = _b.method;
        console.log(responseType, method);
        var selectedRes = (function () {
            switch (true) {
                case (typeof responseType === 'string' && !method):
                    return res
                        .filter(function (_a) {
                        var r = _a.request;
                        return r.returnType ? r.returnType === responseType : false;
                    });
                case (typeof method === 'string' && !responseType):
                    return res
                        .filter(function (_a) {
                        var request = _a.request;
                        return request.method === method;
                    });
                case (typeof responseType === 'string' && typeof method === 'string'):
                    return res
                        .filter(function (_a) {
                        var request = _a.request;
                        return request.method === method;
...
var handleRequest = function (token, request) {
    return request.mergeAll()
        ...
        .flatMap(function (_a) {
        var method = _a.method, multipart = _a.multipart, returnType = _a.returnType, query = _a.options;
        return api_request_1.makeAPIRequest({ returnType: returnType, token: token, method: method, query: query }, multipart);
    });
};
