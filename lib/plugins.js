...
var testType = ramda_1.curryN(2, function (update, _a) {
    var type = _a.type;
    return type ? type.is(update) : false;
});
...
var transform = function (plugins, sources, update, pluginNotFound) { return ramda_1.map(isolatePlugin(update, sources, getQuery(update)), ramda_1.when(ramda_1.isEmpty, function () { return [pluginNotFound]; }, plugins)); };
var makeComponentSelector = ramda_1.curryN(4, function (f, update, plugins, sources) { return ramda_1.when(ramda_1.isNil, function () { return []; }, transform(f(getQuery(update), update, plugins), sources, update, ramda_1.last(plugins))); });
var checkTypeAndQuery = ramda_1.curryN(2, function (update, query) { return ramda_1.cond([
    [ramda_1.allPass([ramda_1.prop('type'), ramda_1.prop('pattern')]), ramda_1.both(testType(update), testPattern(query))],
    [ramda_1.prop('type'), testType(update)],
    [ramda_1.prop('pattern'), testPattern(query)],
    [ramda_1.T, ramda_1.F]
]); });
var toComponents = makeComponentSelector(function (query, update, plugins) {
    return ramda_1.filter(checkTypeAndQuery(update, query), plugins);
});
var toComponent = makeComponentSelector(function (query, update, plugins) {
    return [ramda_1.find(checkTypeAndQuery(update, query), plugins)];
});
