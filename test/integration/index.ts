...
test('should send photo with basic driver', t => {
  ...
  let main = () => ({
    bot: $.from([
      $.just(sendPhoto(
        { chat_id: GROUP_ID, photo: fs.createReadStream(path.join(__dirname, 'fixtures', 'test.jpg')) },
        {}))
    ])
  })
  let { sources, run } = Cycle(main, { bot: basicDriver })
  ...
  let main = () => ({
    bot: $.from([
      $.just(sendAudio(
        {
          chat_id: GROUP_ID,
          audio: fs.createReadStream(path.join(__dirname, 'fixtures', 'test.ogg')),
          performer: 'Skiessi',
...
test.skip('should send document with basic driver', t => {
  ...
  let main = () => ({
    bot: $.from([
      $.just(sendDocument(
        { chat_id: GROUP_ID, document: fs.createReadStream(path.join(__dirname, 'fixtures', 'test.jpg')) },
        {}))
    ])
  })
  let { sources, run } = Cycle(main, { bot: basicDriver })
...
test('should get chat administrators with basic driver', t => {
  ...
  let main = () => ({
    bot: $.from([
      $.just(getChatAdministrators({ chat_id: GROUP_ID }, {}))
    ])
  })
  let { sources, run } = Cycle(main, { bot: basicDriver })
...
test('should get chat members count with basic driver', t => {
  ...
  let main = () => ({
    bot: $.from([
      $.just(getChatMembersCount({ chat_id: GROUP_ID }, {}))
    ])
  })
  let { sources, run } = Cycle(main, { bot: basicDriver })
...
test('should get chat member with basic driver', t => {
  ...
  let main = () => ({
    bot: $.from([
      $.just(getChatMember({ chat_id: GROUP_ID, user_id: 39759851 }, {}))
    ])
  })
  let { sources, run } = Cycle(main, { bot: basicDriver })
  ...
  let main = ({ bot }: Sources) => ({
    bot: $.from([
      bot.events('message')
        .do(() => sources.bot.dispose())
        .map(sendPhoto({
          photo: fs.createReadStream(path.join(__dirname, 'fixtures', 'test.jpg')),
          caption: 'Cycle.js'
        })),
      bot.responses
        .take(1)
...
test('should leave chat with basic driver', t => {
  ...
  let main = () => ({
    bot: $.from([
      $.just(leaveChat({ chat_id: GROUP_ID }, {}))
    ])
  })
  let { sources, run } = Cycle(main, { bot: basicDriver })
