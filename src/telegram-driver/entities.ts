...
export let entityIs =
  curryN(2, (type: string, update: Update): boolean => pipe(
...
export let getEntityFirst =
  curryN(2, (type: string, update: Update) => pipe(
    view(lensPath(['message', 'entities'])),
    defaultTo([]),
    find(propEq('type', type)))(update))

export let getEntityFirstValue =
  curryN(2, (type: string, update: Update) => {
